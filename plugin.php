...
function ig_manage_jogos_shortcode()
{
    ...
    ?> <div class="ig-manage-container"> <?php echo $output; ?> <div class="ig-form-section"> <h3>Adicionar Novo Jogo</h3> <form method="post"> <input type="hidden" name="action" value="add_jogo"> <?php wp_nonce_field(
     "ig_manage_action",
     "ig_manage_nonce"
 ); ?> <p>
     ...
     <input name="nome-eq" id="ig_nome_equipe" type="text" required style="width: 100%;"></p> 
     ...
     <input name="nome" id="ig_nome_jogo" type="text" required style="width: 100%;"></p> 
     <p>
         
         <button type="submit">Cadastrar Jogo</button></p> </form> </div> 
 ...
 <h3>Jogos Cadastrados</h3> <?php if (
     ...
     $jogos
     as $j
 ): ?> <li> <div id="jogo-details-<?php echo $j->id; ?>"><span class="item-name"><?php echo esc_html(
    $j->nome
); ?></span></div> <div class="ig-edit-form" id="edit-form-jogo-<?php echo $j->id; ?>"> <form method="post" style="display:inline-flex; gap: 5px;"> <input type="hidden" name="action" value="update_jogo"><input type="hidden" name="jogo_id" value="<?php echo $j->id; ?>"> <?php wp_nonce_field(
    "ig_manage_action",
    "ig_manage_nonce"
); ?> <input type="text" name="nome" value="<?php echo esc_attr(
     $j->nome
 ); ?>" required> <button type="submit">Salvar</button><button type="button" class="cancel-edit-btn" data-id="<?php echo $j->id; ?>" data-type="jogo">Cancelar</button> </form> </div> <div> <button type="button" class="edit-btn" data-id="<?php echo $j->id; ?>" data-type="jogo">Editar</button> <form method="post" style="display: inline;" onsubmit="return confirm('Tem certeza?');"> <input type="hidden" name="action" value="delete_jogo"><input type="hidden" name="jogo_id" value="<?php echo $j->id; ?>"> <?php wp_nonce_field(
    "ig_manage_action",
    "ig_manage_nonce"
); ?> <button type="submit">Excluir</button> </form> </div> </li> <?php endforeach; ?> </ul> <?php else:echo "<p>Nenhum jogo cadastrado.</p>";endif; ?> </div> </div> <?php
...
function ig_manage_criterios_shortcode()
{
    ...
    ob_start();
    echo ig_get_management_styles();
    ?> <div class="ig-manage-container"> <?php echo $output; ?> <div class="ig-form-section"> <h3>Adicionar Novo Critério</h3> <p><strong>Importante:</strong> Cadastre os critérios na ordem exata que será usada para o desempate.</p> <form method="post"> <input type="hidden" name="action" value="add_criterio"> <?php wp_nonce_field(
     "ig_manage_action",
     "ig_manage_nonce"
 ); ?> <p><label>Nome do Critério</label><br><input name="nome" type="text" required style="width: 100%;"></p> <p><label>Pontuação Máxima (Peso)</label><br><input name="peso" type="number" step="1" value="20" required style="width: 100%;"></p> <p><button type="submit">Cadastrar Critério</button></p> </form> </div> <div class="ig-list-section"> <h3>Critérios Cadastrados (Ordem de Desempate)</h3> <?php if (
     $criterios
    ...
    "ig_manage_action",
    "ig_manage_nonce"
); ?> <button type="submit">Excluir</button> </form> </li> <?php endforeach; ?> </ol> <?php else:echo "<p>Nenhum critério cadastrado.</p>";endif; ?> </div> </div> <?php return ob_get_clean();
}
...
function ig_resultados_page()
{
    ...
    ob_start();
    ?> <div class="wrap"> <h1>Resultados da Mostra de Jogos</h1> <table class="wp-list-table widefat fixed striped"> <thead> <tr> <th style="width: 50px;">Pos.</th><th>Jogo</th><th style="width: 150px;">Pontos (0-100)</th><th style="width: 100px;">Ações</th> </tr> </thead> <tbody> <?php foreach (
     $results
     as $index => $res
 ): ?> <tr> <td><?php echo $index + 1 . "º"; ?></td> <td><?php echo esc_html(
    $res["jogo_name"]
); ?></td> <td><?php echo number_format(
    $res["total_score"],
    2
); ?></td> <td><button class="button-link" onclick="toggleDetails(<?php echo $res[
    "jogo_id"
]; ?>)">Detalhes</button></td> </tr> 
<tr class="details-row" style="display:none;" id="details-<?php echo $res[
    "jogo_id"
]; ?>">
...
</tr> <?php endforeach; ?> </tbody> </table> </div> <script>function toggleDetails(jogoId) { const row = document.getElementById('details-' + jogoId); row.style.display = row.style.display === 'none' ? 'table-row' : 'none'; }</script> <?php return ob_get_clean();
}
...
function ig_resultados_publicos_shortcode()
{
    ...
    ?>
    <style>
        .ig-public-results-wrapper { font-family: sans-serif; max-width: 900px; margin: 40px auto; text-align: center; }
        .ig-podium { display: flex; align-items: flex-end; justify-content: center; gap: 15px; margin-top: 50px; padding: 20px 0; border-bottom: 2px solid #eee; }
        .podium-place { width: 30%; max-width: 250px; padding: 20px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); background: #f9f9f9; }
        .podium-place h3 { margin: 0 0 10px; font-size: 1.8em; color: #333; }
        .podium-place .game-name { font-size: 1.4em; font-weight: bold; margin-bottom: 10px; min-height: 60px; }
        .podium-place .score { font-size: 2.2em; font-weight: bold; color: #111; }
        .podium-place.first { order: 2; height: 280px; background: linear-gradient(145deg, #ffd700, #f0c400); border: 3px solid #ffc107; }
        .podium-place.second { order: 1; height: 240px; background: linear-gradient(145deg, #e0e0e0, #cccccc); border: 3px solid #c0c0c0; }
        .podium-place.third { order: 3; height: 200px; background: linear-gradient(145deg, #d2a679, #c4915c); border: 3px solid #cd7f32; }
        .ig-other-ranks { margin-top: 40px; text-align: left; }
        .ig-other-ranks h2 { text-align: center; color: #444; }
        .ig-other-ranks ol { list-style-type: none; counter-reset: rank-counter <?php echo count(
            $results
        ) > 3
            ? 3
            : 0; ?>; padding: 0; }
        .ig-other-ranks li { background: #fff; border: 1px solid #ddd; padding: 15px; margin-bottom: 10px; border-radius: 5px; display: flex; justify-content: space-between; align-items: center; font-size: 1.1em; }
        .ig-other-ranks li::before { content: counter(rank-counter) "º"; counter-increment: rank-counter; font-weight: bold; color: #4f1299; margin-right: 15px; }
        .ig-other-ranks .game-name { font-weight: bold; }
        .ig-other-ranks .score { font-weight: bold; color: #333; }
        @media (max-width: 768px) {
            .ig-podium { flex-direction: column; align-items: center; }
            .podium-place { width: 80%; order: 0 !important; height: auto !important; margin-bottom: 15px; }
            .podium-place.first { order: -2 !important; }
            .podium-place.second { order: -1 !important; }
        }
    </style>
...
            <form id="ig-cpf-form"><label for="ig_cpf">CPF:</label><input type="text" id="ig_cpf" name="ig_cpf" required placeholder="000.000.000-00" maxlength="14"><button type="submit" class="init-button">Votar</button><div id="ig-message-area" style="margin-top: 10px; font-weight: bold;"></div></form>
        </div>
        <div id="ig-evaluation-content" style="display: none;"></div>
    </div>
    <style>
        #ig-cpf-form-container, #ig-location-check-container { text-align: center; max-width: 400px; margin: 0 auto; padding: 20px; }
        #ig-cpf-form { display: flex; flex-direction: column; align-items: center; gap: 15px; }
        #ig_cpf { width: 100%; padding: 15px; font-size: 1.2em; box-sizing: border-box; text-align: center; }
        .init-button { width: 40%; padding: 15px; font-size: 1em; font-weight: bold; border: none; border-radius: 5px; cursor: pointer; background-color: #4f1299; color: white; }
        .lista-jogos-publica { list-style: none; padding: 0; } .lista-jogos-publica li { padding: 10px; border: 1px solid #ccc; margin-bottom: 5px; border-radius: 4px; } .lista-jogos-publica li.ja-avaliado { background-color: #f0f0f0; color: #888; } .lista-jogos-publica li.ja-avaliado a { text-decoration: none; pointer-events: none; } .lista-jogos-publica li a { text-decoration: none; color: #000; font-weight: bold; cursor: pointer; } .ig-crit { border: 1px solid #ccc; padding: 10px; margin-bottom: 10px; border-radius: 5px; } .ig-stars { display: flex; cursor: pointer; } .ig-stars svg { width: 30px; height: 30px; } .star-path-full { fill: #ffd700; } .star-path-empty { fill: #ccc; } #finalize-evaluation-btn { background: linear-gradient(90deg,#9c27be 0%, #ff6cc7 100%); color: white; border: none;font-size: 16px; cursor: pointer; border-radius: 5px; } #finalize-evaluation-btn:hover { background-color: #b71c1c; }
    </style>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleLocationError(error) {
        let message = '';
        switch (error.code) {
            case error.PERMISSION_DENIED: message = "Você precisa permitir o acesso à localização para poder votar."; break;
            case error.POSITION_UNAVAILABLE: message = "Não foi possível obter sua localização atual."; break;
            case error.TIMEOUT: message = "A verificação de localização demorou muito. Tente novamente."; break;
            default: message = "Ocorreu um erro desconhecido ao verificar a localização."; break;
        }
    ...
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(handleLocationSuccess, handleLocationError, { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 });
    } else {
    ...
    contentContainer.addEventListener("click", function(e) {
        ...
        if (jogoLink) {
            ...
            if (confirm("Deseja encerrar a sua avaliação? Após confirmar, você não poderá mais usar este CPF para avaliar ou alterar seus votos.")) {
                ...
                const body = new URLSearchParams({ action: 'ig_finalize_cpf', cpf: avaliadorCPF });
                fetch("<?php echo admin_url(
                    "admin-ajax.php"
                ); ?>", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: body.toString() })
                .then(res => res.json())
...
function ig_controle_votacao_shortcode() {
    ...
    ?>
    <style>
        .ig-control-wrapper { max-width: 600px; margin: 40px auto; padding: 30px; border: 1px solid #ccc; border-radius: 8px; text-align: center; font-family: sans-serif; }
        .ig-control-wrapper h2 { margin-top: 0; }
        .ig-status { margin: 20px 0; font-size: 1.2em; }
        .ig-status-aberta { font-weight: bold; color: #28a745; }
        .ig-status-encerrada { font-weight: bold; color: #dc3545; }
        .ig-control-button { font-size: 1.3em; padding: 15px 30px; border: none; border-radius: 5px; cursor: pointer; color: white; }
        .ig-button-encerrar { background-color: #dc3545; }
        .ig-button-reabrir { background-color: #28a745; }
        .ig-notice {padding:10px 15px;border-radius:5px;margin-bottom:15px; text-align: left;}
        .ig-notice-success {background-color:#d4edda;color:#155724}
    </style>
        ...
        <div id="ig-star-form">
            ...
            <div class="ig-crit">
                ...
                <div class="ig-stars" data-crit-id="<?php echo esc_attr(
                    ...
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                    <svg class="star" data-value="<?php echo $i; ?>" viewBox="0 0 24 24"><path class="star-path-empty" d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
                    <?php endfor; ?>
                </div>
            </div>
            <?php endforeach; ?>
...
function ig_render_ifsp_evaluation_shortcode()
{
    ...
    if (isset($_GET["jogo_id"])) {
        ...
        ob_start();
        ?> <div class="ig-ifsp-eval-container"> <h3>Avaliando o jogo: <strong><?php echo esc_html(
     $jogo->nome
 ); ?></strong></h3> <p>Avaliador: <?php echo esc_html(
    $current_user->display_name
); ?></p> <p>Dê sua nota de 0 a 5 para cada critério abaixo:</p> <div id="ig-star-form-ifsp"> <?php foreach (
    $criterios
    as $c
):
    ...
    ?> <div class="ig-crit"> <strong><?php echo esc_html(
     $c->nome
 ); ?></strong> <div class="ig-stars" data-crit-id="<?php echo esc_attr(
    $c->id
); ?>" data-rating="<?php echo $rating; ?>"> <?php for ($i = 1; $i <= 5; $i++):
    $cls =
        $rating >= $i
            ? "star-path-full"
            : "star-path-empty"; ?> <svg class="star" data-value="<?php echo $i; ?>" viewBox="0 0 24 24"><path class="<?php echo $cls; ?>" d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg> <?php
endfor; ?> </div> </div> <?php
endforeach; ?> <a href="<?php echo esc_url(
     get_permalink()
 ); ?>">Voltar para a lista de jogos</a> </div> </div> <style>.ig-crit{border:1px solid #ccc;padding:10px;margin-bottom:10px;border-radius:5px}.ig-stars{display:flex;cursor:pointer}.ig-stars svg{width:30px;height:30px}.star-path-full{fill:#ffd700}.star-path-empty{fill:#ccc}</style> <script> document.querySelectorAll('#ig-star-form-ifsp .ig-stars').forEach(container => { container.addEventListener('click', e => { const star = e.target.closest('svg'); if (!star) return; const value = parseInt(star.dataset.value, 10); container.dataset.rating = value; container.querySelectorAll('svg path').forEach((p, i) => { p.className.baseVal = (i < value) ? 'star-path-full' : 'star-path-empty'; }); const critId = container.dataset.critId; fetch('<?php echo admin_url(
    "admin-ajax.php"
); ?>', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: `action=ig_save_ifsp_eval&_ajax_nonce=<?php echo wp_create_nonce(
    "ig_save_ifsp_eval_nonce"
); ?>&jogo_id=<?php echo $jogo_id; ?>&criterio_id=${critId}&nota=${value}` }); }); }); </script> <?php return ob_get_clean();
    } else {
        ob_start(); ?> <div class="ig-ifsp-eval-container"> <h2>Área do Avaliador</h2> <p>Olá, <strong><?php echo esc_html(
     $current_user->display_name
 ); ?></strong>. Por favor, escolha um jogo para avaliar:</p> <ul> <?php foreach (
    $jogos
    as $jogo
    ...
    $jogo->nome
); ?></a></li> <?php endforeach; ?> </ul> </div> <?php return ob_get_clean();
    }
}
